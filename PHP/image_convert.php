...
function exec_img_convert($srcPic, $dstPic, $scale = "100x100") {
	...
	if ($srcW < $scaleW / $maxDiv || $srcH < $scaleH / $maxDiv) {
		//if the resolution of src is less than the scale,return
		return false;
	}
	$similar_degree = ($srcW * 1.0 / $srcH) - ($scaleW * 1.0 / $scaleH);
	if ($similar_degree < 0.00000001 && $similar_degree > 0) {
		// equal proportion
		$srcH_new = $srcH;
		$srcW_new = $srcW;
		$cropH    = 0;
		$cropW    = 0;
	} elseif ($similar_degree < 0) {
		// crop height
