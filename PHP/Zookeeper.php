...
class Zookeeper_Handler {
     ...
     * @return string|null
     */
    public function watchChildren($path, $callback)
    {
        if (!is_callable($callback)) {
            return null;
        }
        ...
        if ($this->zookeeper->exists($path)) {
            ...
            if (!in_array($callback, $this->callback[$path])) {
                $this->callback[$path][] = $callback;
                return $this->zookeeper->getChildren($path, [$this, 'watchCallback_getChildren']);
            }
        }
    }
    ...
    public function watchCallback_getChildren($event_type, $stat, $path) {
        ...
        foreach ($this->callback[$path] as $callback) {
            //因为监听中用过一次，需要重新注册一次新的监听
            $this->zookeeper->getChildren($path, [$this, 'watchCallback_getChildren']);
             //对path="/signal_server"，执行前面已经注册函数（callback_children）
...
$zk = new Zookeeper_Handler('127.0.0.1:2181');
var_dump($zk->get('/'));
